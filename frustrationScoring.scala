package com.inceptez.retail.analytics
...
{
    case class rddtoes (customernumber: String,customername:String,city:String,
                        creditlimit:String,MSRP:String,orderdate:String);
    
    /* function to connect and pull data from mysql database */
    def getRdbmsData(sqlc: SQLContext,DatabaseName: String, TableName: String, 
                     PartitionColumn:String, ConnFile: String):DataFrame= 
    {
    ...
    def processCustPayData(sqlc: HiveContext) = 
    {
        ...
        sqlc.sql("""select customerNumber,customerName,concat(contactFirstName,' ',contactLastName) as
        contactfullname,addressLine1,city,state,postalCode,country,phone,creditLimit,checknumber,amount,paymentdate from
        cust_payment """).createOrReplaceTempView("custdetcomplextypes")
        
        //processing order details and registering as orddetcomplextypes
        sqlc.sql("""select customernumber,ordernumber,shippeddate,status,comments,productcode,quantityordered,priceeach,orderlinenumber,
                    productName,productLine,productScale,productVendor,productDescription,quantityInStock,buyPrice,MSRP,orderdate 
                    from order_products""").createOrReplaceTempView("orddetcomplextypes")
        ...
        val custpaymentcomplextypes=sqlc.sql("""select CONCAT(customerNumber,'~',checknumber,'~',CONCAT(creditLimit,'$',amount),'~',paymentdate) 
                                                from cust_payment""")
        custpaymentcomplextypes.write.mode(SaveMode.Overwrite).option("header","true")
                               .csv("hdfs://localhost:54310/user/hduser/output/CustPayment")
        ...
        sqlc.sql("""create external table IF NOT EXISTS retail_mart.custordfinal (customernumber STRING, customername
        ...
        val custfinal=sqlc.sql("""insert into retail_mart.custordfinal select cd.customernumber customernumber,cd.customername
        customlername,cd.contactfullname contactfullname,cd.addressLine1 addressLine1,cd.city city,cd.state state,cd.country
        ...
        val custfinales = sqlc.sql("""select cd.customernumber customernumber,cd.customername customername,
                                      cd.city city,cast(o.quantityInStock as int) quantityInStock,cast(o.MSRP as double) MSRP,
                                      cast(o.orderdate as date) orderdate from custdetcomplextypes cd inner join orddetcomplextypes o 
    ...
    def writetoes (custfinal:DataFrame) = 
    {
        ...
        val toes = custfinaltoes.map(x => rddtoes(x(0).toString(),x(1).toString(), x(2).toString,
                                                  x(3).toString,x(4).toString(),x(5).toString()))
        custfinal.saveToEs("custfinaltoes2/doc1");
    }
    
    def main( args: Array[String]) 
    {
        val conf = new SparkConf().setAppName("Frustration_Scoring_SQL_Hive")
        ...
        val custSql="""(select c.customerNumber, upper(c.customerName) as customerName,c.contactFirstName,
        ...
        val ordSql="""(select o.customernumber,o.ordernumber,o.orderdate as
        orderdate,o.shippeddate,o.status,o.comments,od.quantityordered,od.priceeach,
        od.orderlinenumber,p.productCode,p.productName,p.productLine,p.productScale,p.productVendor,p.productDescription,
        p.quantityInStock,p.buyPrice,p.MSRP from orders o inner join orderdetails od on o.ordernumber=od.ordernumber inner join
        ...
        println("Customer payment details availabe in /user/hduser/output/CustPayment file")
        sc.stop()
    }
}
